...
for i in $pre1 $pre2a $pre2b $pre2c \
    $test1 $test2 $test3 $test4 $test5 $test6 $f_input; do
...
if test X"$1" = X &&
   ...
   test -r "$ref6"; then
	...
	echo "Test six..." >&2
	osh $test6 ONE_TWO_THREE_FOUR_FIVE 2>&1 | diff $ref6 -
	...
	if rm -f $pre1ref $done $ref1 $ref2 $ref3 $ref6; then
		osh $pre1 >$pre1ref 2>&1
		osh $test1 2>&1 | sed 's/^[0-9][0-9]*/?????/' >$ref1
		osh $test2 >$ref2 2>&1
		osh $test3 >$ref3 2>&1
		osh $test6 ONE_TWO_THREE_FOUR_FIVE >$ref6 2>&1

		if chmod 0444 $pre1ref $ref1 $ref2 $ref3 $ref6; then
			echo "Done." >&2
			exit 0
		fi
	fi
