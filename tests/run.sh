...
test2=test2.osh		# All command syntax is bad.
 log2=test2.log
test3=test3.osh		# Test cannot open and cannot create.
 log3=test3.log
test4=test4.osh		# Test 'Command line overflow' errors.
 log4=test4.log
test5=test5.osh		# Test 'Too many characters' errors.
 log5=test5.log
test6=test6.osh		# Test redirection arguments w/ built-in commands.
####
...
# Create the other required files if they do not already exist.
test -e "$fcreat" || {
...
if ! $NEWREF &&
   ...
   test -r "$log4" && test -r "$log5"; then
	...
	echo "Test one (takes at least 5 seconds)..." >&2
	$SH $test1 $SH $IF $GOTO 2>&1 | \
		sed 's/^[0-9][0-9]*/?????/' | diff $log1 -
	...
	echo "Test two..." >&2
	$SH $test2 $SH $IF $GOTO 2>&1 | diff $log2 -
	...
	echo "Test three..." >&2
	$SH $test3 $SH $IF $GOTO 2>&1 | diff $log3 -
	...
	if rm -f $prelog1 $log1 $log2 $log3 $log4 $log5; then
		...
		$SH $test1 $SH $IF $GOTO 2>&1 | \
			sed 's/^[0-9][0-9]*/?????/' >$log1
		$SH $test2 $SH $IF $GOTO >$log2 2>&1
		$SH $test3 $SH $IF $GOTO >$log3 2>&1
