...
# Where does login(1) live on your system?  This is the utility
# that will be used by the shell's login special command to replace
# an interactive shell with an new instance of login.
#
...
SHSRC=		osh.h main.c parse.c exec.c
SHOBJ=		main.o parse.o exec.o
...
osh: $(SHSRC)
	@if test -e main.o && \
		! ( strings -a main.o | grep OSH_COMPAT >/dev/null ); then \
			rm -f $(SHOBJ); \
	fi
	@$(MAKE) $@x
...
check: all
	@$(SHELL) tests/run.sh $(PROGS)

check-sh6: sh6 if goto
	@$(SHELL) tests/run.sh sh6 if goto

check-newref: all
	@$(SHELL) tests/run.sh -newref $(PROGS)
