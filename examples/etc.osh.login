...
: " so helps to avoid the possibility of unexpected errors and lets "
: " you know exactly why your initialization files are not working  "
: " as you might expect them to work.                               "
:
goto HAVE_GOTO
	echo
	echo "/etc/osh.login: goto utility required (terminating now)"
	echo
	sleep 5; false; exit

: HAVE_GOTO
	if x$s = x0 goto HAVE_IF
	echo
	echo "/etc/osh.login: if utility required (terminating now)"
	echo
	sleep 5; false; exit
...
if x$u = xroot goto SUPATH
	:
	: " Use this as the default PATH for regular users. "
	:
	setenv	PATH	/usr/bin:/bin:/usr/X11R6/bin:/usr/local/bin
	goto JUMP1
...
: JUMP3
	: " Reset the ignored signals. "; trap - 1 2 3 15
